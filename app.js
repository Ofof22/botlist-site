...
app.get("/sitemap.xml", async function(req,res) {
  ...
  const renderTemplate = (res, req, template, data = {}) => {
          ...
          if(req.isAuthenticated()) {
              ...
              if(uyevarmikiaq) {
                  ...
                  next();
                      ...
                      res.redirect('/error?code=200&message=Sitemiz Geçiçi Olarak Bakımdadır!') 
                  }
              } else {
                  res.redirect('/error?code=200&message=Sitemiz Geçiçi Olarak Bakımdadır!') 
              }
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://cliff-delightful-ambulance.glitch.me/user/${a.id}) ** Adlı Kullanıcı Siteye Giriş Yaptı! ** Kullanıcı Siteden Banlı Olduğu İçin Giriş Yapamadı!.`).addField("Kullanıcı İsmi", a.username).addField("İd", a.id).addField("Etiketi", a.discriminator))
        })
        req.session.destroy(() => {
        ...
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://cliff-delightful-ambulance.glitch.me/user/${a.id}) Adlı Kullanıcı ** site** giriş yaptı.`).addField("Kullanıcı İsmi", a.username).addField(" ID", a.id).addField("Etiketi", a.discriminator))
    ...
    app.post("/bot/:botID/edit", checkMaintence, checkAuth, async (req,res) => {
     ...
     let botdata = await botsdata.findOne({ botID: req.params.botID });
           client.users.fetch(botdata.ownerID).then(sahip => {
               client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>  **${botdata.username}** Adlı Botun Red Edildi `)
       ...
       app.post("/admin/certificate/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
         ...
         let botdata = await botsdata.findOne({ botID: req.params.botID });
          client.users.fetch(botdata.botID).then(bot => {
              client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> Adlı Kullanıcının **${bot.tag}** Adlı Botunun Sertifika Başvurusu Red Edildi!`)
              client.users.cache.get(botdata.ownerID).send(` **${bot.tag}** Adlı Botun Sertifika Başvurusu Red Edildi!\nSebep: **${rBody['reason']}**`)
          });
          await appsdata.deleteOne({ botID: req.params.botID })
          let guild = client.guilds.cache.get(settingsc.serverID)
      ...
      app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`<a:maintence:833375738547535913> Wynex Code Nedeniyle Değiştirildi **${req.body.reason}**`).then(a => { 
      ...
      app.post("/admin/unmaintence",  checkAdmin, checkAuth, async (req, res) => {
        ...
        .setFooter("Wynex Code © Tüm Hakları Saklıdır");
